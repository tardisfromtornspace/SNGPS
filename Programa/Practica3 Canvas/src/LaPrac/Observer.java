...
public class Observer implements IObserver {
	...
	public void actualizar() {
		...
		if(!parseada.get(0).equals("$GPGGA") || parseada.size() != 15) {
			/*if(parseada.get(0).equals("$GPRMC")) {
			    if (parseada.get(1).equals(""))
				    return;
				System.out.println("Campos parseo :"+ parseada);
				oesteEste = parseada.get(6);
				norteSur = parseada.get(4);
				
				if (parseada.get(4).length() > 0) {
					valor = Double.parseDouble(parseada.get(5));
				}
				longitud = longitudaGrados(valor);
				
				valor = 0.0;
				if (parseada.get(2).length() > 0) {
					valor = Double.parseDouble(parseada.get(3));
				}
				latitud = latitudaGrados(valor);
				
				
				velocidad = Double.parseDouble(parseada.get(7)) * nudosaKmH; // Nos dan la velocidad en nudos, la pasamos a Km/h
				angulo = Double.parseDouble(parseada.get(8)); // Nos la dan en grados
		...
		if(parseada.get(0).equals("$GPRMC")) {
			System.out.println("Como trama RMC, obtengo la velocidad obtenida y la comparo");
			...
			angulo = Math.asin(vecVel[0]/(velocidad * difTiempo*1000)); 
		...
		if(velocidad < 0.9 * this.limite) {
			// Pintar rectángulo verde TO-DO preguntar a profe sobre interfaz
			this.canvas.setMiColor(Color.GREEN);
		} else if (velocidad < 1.1 * this.limite) {
			// Pintar rectángulo amarillo TO-DO preguntar a profe sobre interfaz
	...
	public void test() { // Esto es para pruebas de cálculos según el ejemplo de errata, debería salir lo mismo
		...
		//double longitud = longitudaGrados(337.7013); // Lo de la práctica 1, el otro ejemplo, se ve que funciona
		...
		double ajuste = 0.5/60; // El ajuste es porque las coordenadas del mapa se tomaron a ojímetro
		

		
		double longitud = longitudaGrados(337.9079); // Coordenadas inicio de INSIA
