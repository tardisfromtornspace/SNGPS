...
import LaPrac.VecUtil.Vec2;

public class Observer implements IObserver {
	Semaphore mySemaphore;
	private volatile boolean myAcquireLock = false;
	VecUtil<Double> vu;
	ArrayList<Vec2<Double>> misSujetosPrevLocation;
	ArrayList<Vec2<Double>> misSujetosRelDist;
	ArrayList<Double> misSujetosAbsDist;
	...
	public Observer(int huso, String tipo, MyFrame frame, double precision) {
		if (tipo.equalsIgnoreCase("Europeo")) {
			calculosIniciales(aEuropeo, fEuropeo, e2Europeo, huso);
		} else {
			calculosIniciales(aWGS84, fWGS84, e2WGS84, huso);
		}
		this.vu = new VecUtil<>(precision);
		this.misSujetos = new ArrayList<>();
		this.misSujetosPrevLocation = new ArrayList<>();
		this.misSujetosRelDist = new ArrayList<>();
		this.misSujetosAbsDist = new ArrayList<>();
		this.mySemaphore = new Semaphore(0);
		this.frame = frame;
	}
	public void startSpeedTracer()
	{
		synchronized (this) {
			while(true) {
				try {
					Thread.sleep(1750);
					this.frame.updateSpeedometerAll(getCurrentSubjectsDist(), 1.75d);
	...
	public void addObservable(IObservable sujeto)
	{
		...
		this.misSujetosPrevLocation.add(VecUtil.getDefaultVec());
		this.misSujetosRelDist.add(VecUtil.getDefaultVec());
		this.misSujetosAbsDist.add(0.0d);
		this.mySemaphore.release(1);
	}
	...
	public double getRotationfromUnitVec(Vec2<Double> v)
	{
		double deg = Math.toDegrees(Math.atan(v.getY()/v.getX()));
		if(v.getX() > 0.d)
			deg = deg + 180;
	...
	public void actualizar(int issuer) {
		...
		String cadena = misSujetos.get(issuer).getMensaje(); //""; // miSujeto.getMensaje(); // Esto se ajustar�a a lo sacado del Subject
		...
		double ajuste = 0.d/60; // Ajuste del problema de coordenadas a ojímetro, hicimos un pequeño error de medio segundo en latitud, esto lo corrije
		if (this.primeraVez) {
			this.primeraVez = false;
			establecimientoCoordIniciales(338.083, "W", 4023.500, "N", 337.417, "W", 4023.100,"N", ajuste, 0.0, this.huso, this.huso);
			
		}
		Vec2<Double> diferenciaDeCoordenadas = new Vec2<>(resultado[0] -this.coordInicialesEste, -(resultado[1] -this.coordInicialesNorte)); // Este, Norte	
		Vec2<Double> w = vu.getVec2Subv(this.misSujetosPrevLocation.get(issuer), diferenciaDeCoordenadas);
		
		
		this.misSujetosPrevLocation.set(issuer, diferenciaDeCoordenadas);
		
		if(!vu.nearAbsoluteZero(w))
			this.frame.setLinkedPointRotation(issuer, getRotationfromUnitVec(w));
		this.frame.setLinkedPointRelativeLocation(
				issuer,
				diferenciaDeCoordenadas.getX()/this.diferenciaEste,
				diferenciaDeCoordenadas.getY() /this.diferenciaNorte
				);
	
		try {
			while(this.myAcquireLock) {
				Thread.sleep(0, 500);
			}
			mySemaphore.acquire();
			this.misSujetosRelDist.set(issuer, vu.getVec2Addv(this.misSujetosRelDist.get(issuer), w));
			this.misSujetosAbsDist.set(issuer, this.misSujetosAbsDist.get(issuer) + vu.getVec2DMod(w));
			mySemaphore.release();
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
	...
	public void resetCurrentSubjectsDist()
	{
		int len = this.misSujetosRelDist.size();
		for(int i = 0; i < len; i++) {
			this.misSujetosRelDist.set(i, VecUtil.getDefaultVec());
			this.misSujetosAbsDist.set(i, 0.0d);
		}
	}
	
	public ArrayList<Double> getCurrentSubjectsDist()
	{
		ArrayList<Double> get = new ArrayList<>();
		try {
			this.myAcquireLock = true;
			this.mySemaphore.acquire(misSujetos.size());
			for(double v:this.misSujetosAbsDist)
				get.add(v);
			resetCurrentSubjectsDist();
			mySemaphore.release(this.misSujetos.size());
			this.myAcquireLock = false;			
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
		return get;
	...
	public void test() { // Esto es para pruebas de c�lculos seg�n el ejemplo de errata, deber�a salir lo mismo
		// Tomo el dato, y lo parseo
		// Luego ajusto con f�rmulas
		String cadena = misSujetos.get(0).getMensaje(); //""; // miSujeto.getMensaje(); // Esto se ajustar�a a lo sacado del Subject
		System.out.println("Cadena GPS: " + cadena);
		...
		//double longitud = longitudaGrados(337.619); // Lo de la pr�ctica 1, el ejemplo, se ve que funciona
		//double latitud = latitudaGrados(4023.429); 
		
		//double longitud = longitudaGrados(337.7013); // Lo de la pr�ctica 1, el ejemplo, se ve que funciona
	...
	// Seg�n la foto comenzamos en 40� 23' 33'' N 3� 38' 04 '' W
	...
	/*
	public double[] coordenadasInicialesCanvas(double longitudOrig, String oesteEste, double latitudOrig, String norteSur) { // Esto es para pruebas de c�lculos seg�n el ejemplo de errata, deber�a salir lo mismo
